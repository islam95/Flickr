...
import java.util.Properties;        // For properties file (stores API key and secret)
...
//  ***************************************************************************
/**
 * Simple program that uses the Flickr4Java package to link with the Flickr API
 * and perform a query.
 *
 * @author Islam Dudaev
...
public class HelloFlickr {
    ...
    private String contributor = new String();
    ...
    /**
     * Performs a simple query of the Flickr service.
     *
    ...
    private boolean doQuery() {
        ...
        String tag = "serena williams";
        criteria.setTags(new String[]{tag});
        try {
            photoList = photos.search(criteria, 250, 1);
        } catch (FlickrException e) {
            System.err.println("Problem performing flickr query: " + e);
            return false;
        }

        System.out.println(photoList.getTotal() + " photos found with tag '" + tag + "'.");
        return true;
    }

    /**
     * Performs a simple georeferencing query of the Flickr service.
     *
     * @return True if query performed successfully.
     */
    private boolean doQuery1() {
        // Provides access to Flickr's 'photos' API calls.
        PhotosInterface photos = flickr.getPhotosInterface();
        ...
        SearchParameters criteria = new SearchParameters();
        criteria.setExtras(Extras.ALL_EXTRAS);    // Needed for geographical searches.
        criteria.setAccuracy(16);       // Limits queries to photos with maximum spatial accuracy.

        // Set spatial bounds of query (strings storing whole number latitude and longitude values).
        String minLongitude = "-5",
                maxLongitude = "2",
                minLatitude = "50",
                maxLatitude = "55";

        criteria.setBBox(minLongitude, minLatitude, maxLongitude, maxLatitude);
        ...
        System.out.println("Geotagged search: " + photoList.getTotal()
                + " photos taken in the last year between ("
                + minLongitude + "," + minLatitude + ") and ("
